...
Tool for matching grant and publication records to [OpenAlex](https://openalex.org) works with two modes:
...
### Title-Based Search
The title-based matching process begins by normalizing the input title to improve matching accuracy. When a title comes in as input, the matching first looks for and extracts any dates that might be embedded in the title text. For example, a title like "9 July 2019, Climate Change Report" becomes simply "Climate Change Report" with the date extracted and stored separately. The normalization then continues on to identifying and removing common title additions, such as subtitles, content in parentheses or brackets, report numbers, version indicators, and suffixes like "abstract" or "preprint". The text is then converted to lowercase, special characters are removed, and Unicode characters are transliterated to their ASCII equivalents. When an aggressive matching mode is used as a final fallback, English stopwords are filtered out as well.

Once the title is normalized, the system searches OpenAlex using a two-stage approach. First, it attempts an exact search using the cleaned title. If this doesn't yield results above the specified similarity threshold, it falls back to a fuzzy search that can handle minor variations in wording. When a publication year is provided, the search is refined by filtering to publications within a ±2 year window. The similarity between titles is calculated using Levenshtein distance.

After potential matches are found, the system validates them through additional checks. If author information is available, it compares the last names of authors from the input with those in the OpenAlex record, requiring at least an 85% similarity match. Only matches that exceed the similarity threshold (default of 95%) are returned as valid matches.

## How Author-Affiliation Matching Works

The author-affiliation matching approach is designed for scenarios where you have author names and institutional affiliations, but may not have any publications associated with the author. This method searches for all publications by specific authors and their institutions, making it useful for things like tracking research outputs from grants where the principal investigator and their institution are known.

The matching process uses an institution-first matching strategy to minimize false positives. When a search begins, the matching first attempts to resolve the institutional affiliation to a specific organization/institution ID in OpenAlex by searching for organizations that match the affiliation string. If no match is found with sufficient confidence, the matching falls back to using the [Research Organization Registry (ROR) API](https://ror.readme.io/docs/rest-api), which specializes in disambiguating messy affiliation strings to standardized institution identifiers.

Once an institution is identified, the matching extracts the author's surname from the input name, handling various formats including "Last, First", "First Last", "Last Initial", as well as compound surnames. It then searches specifically for authors with that surname who are affiliated with the identified institution.

When multiple authors are found at the institution, the matching performs name disambiguation using a configured name matching style. Each name is first normalized by converting Latin characters to ASCII equivalents and compared using Jaro-Winkler similarity scoring. The author with the highest name similarity score above the threshold is selected as the match. The system then retrieves publications where this author is specifically affiliated with the matched institution, filtering out any works from their time at other organizations. For example, if searching for "J Smith at University of Michiga", the system will only return papers where J Smith lists University of Michiga as their affiliation, excluding any papers from their previous affiliations. Works are further filtered by year window if a grant year is provided.

When an author is successfully matched at an institution, but has no publications associated with them for the specified year range, the matching returns an empty result rather than falling back to a broader search. This prevents false positives that could occur from matching similarly-named authors at other institutions.

If the institution cannot be resolved through either OpenAlex or the ROR API, the matching falls back to a broader search strategy. In this fallback mode, it searches for authors by name across all institutions and then validates each potential match by comparing the affiliation strings. This comparison can use either fuzzy text matching or an embedding model (`cometadata/affiliation-clustering-0.3b`) specially trained on institution name disambiguation using a [triplet loss](https://en.wikipedia.org/wiki/Triplet_loss) approach. The embedding model works by generating vector representations of institution names and cosine similarity to identify true matches, allowing it to recognize that "Harvard Medical School" and "Harvard University" refer to related institutions even when the text differs significantly.

Each potential match is scored using a weighted combination of author name similarity (30% by default) and affiliation similarity (70% by default), reflecting the strategy's preference for institutional affiliation to reduce false positives. The matching also requires a minimum affiliation score (85% by default) to consider a match valid. Unlike in the title-based matching, which produces one output row per input record, author-affiliation matching retunrs multiple output rows, one for each publication found for that author at that institution. Each output row includes the match scores for both author and affiliation, allowing the user to assess their confidence in each match.
